...
def torch_predict_temperature(room, constants, T_r, T_a, S_t, heating_setpoint, cooling_setpoint, O):
...
def train_one_day_pytorch(room, df, lr=5e-6, epochs=800):
    ...
    for epoch in range(epochs):
        optimizer.zero_grad()
        T_pred = torch_predict_temperature(room, constants, T_r, T_a, S_t, heating_setpoint, cooling_setpoint, O)
