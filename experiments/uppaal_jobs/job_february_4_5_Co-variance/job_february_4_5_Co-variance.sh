#!/bin/bash
#SBATCH --job-name=4/february_5_+Co-variance
...
cd /nfs/home/student.aau.dk/tb30jn/BlindControl/experiments/uppaal_jobs/job_february_4_5_Co-variance || exit 1
...
' "$escaped_data_content" > data_content_2025-02-21.txt
    ...
    if(( i == 0 )); then
      ...
      temp_timestamp=$(grep -oE '\([0-9]+\.[0-9]+,[0-9]+\.[0-9]+\)' "output_february_4_5_$((i-1)).csv" | tail -n 1)
      init_temp=$(echo "$temp_timestamp" | grep -oE '[0-9]+\.[0-9]+' | tail -n 1)
      
      blocked_timestamp=$(
          grep -A1 '^blocked:' "output_february_4_5_$((i-1)).csv"             | tail -n1             | sed -E 's/.*\[([0-9]+)\]:.*\(([0-9]+\.[0-9]+,[0-9]+\.[0-9]+)\).*/ in ()/'
        )
        ...
        blinds_timestamp=$(
          grep -A1 '^blinds:' "output_february_4_5_$((i-1)).csv"             | tail -n1             | sed -E 's/.*\[([0-9]+)\]:.*\(([0-9]+\.[0-9]+,[0-9]+\.[0-9]+)\).*/ in ()/'
        )
    ...
    verifyta "BlindModel.xml" "uppaal.q" --generate-strategy=1 --learning-method=0 --exploration=1 | tee "output_february_4_5_${i}.csv"
done