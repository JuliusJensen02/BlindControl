#!/bin/bash
#SBATCH --job-name=24/winter_5_+Co-variance
...
cd /nfs/home/student.aau.dk/tb30jn/BlindControl/experiments/uppaal_jobs/job_winter_24_5_Co-variance || exit 1
...
for ((i=0; i<=46; i++)); do
    ...
    python3 -u -m data_uppaal_format --date='2025-02-21'

    data_path="data_arrays_2025-02-21.c"
    data_content=$(< "$data_path")
    escaped_data_content=$(echo "$data_content" | sed 's/[\/&\[\]\]/\&/g')
    printf '%s
' "$escaped_data_content" > data_content_2025-02-21.txt
    sed -i -E '
      /const double data\[\]\[\] = \{\};/ {
        r data_content_2025-02-21.txt
        d
      }
    ' BlindModel.xml
    ...
    if(( i == 0 )); then
      ...
      temp_timestamp=$(grep -oE '\([0-9]+\.[0-9]+,[0-9]+\.[0-9]+\)' "output_winter_24_5_$((i-1)).csv" | tail -n 1)
      init_temp=$(echo "$temp_timestamp" | grep -oE '[0-9]+\.[0-9]+' | tail -n 1)
      
      blocked_timestamp=$(
          grep -A1 '^blocked:' "output_winter_24_5_$((i-1)).csv"             | tail -n1             | sed -E 's/.*\[([0-9]+)\]:.*\(([0-9]+\.[0-9]+,[0-9]+\.[0-9]+)\).*/ in ()/'
        )
        ...
        blinds_timestamp=$(
          grep -A1 '^blinds:' "output_winter_24_5_$((i-1)).csv"             | tail -n1             | sed -E 's/.*\[([0-9]+)\]:.*\(([0-9]+\.[0-9]+,[0-9]+\.[0-9]+)\).*/ in ()/'
        )
    ...
    verifyta "BlindModel.xml" "uppaal.q" --generate-strategy=1 --learning-method=0 --exploration=1 | tee "output_winter_24_5_${i}.csv"
done